...
let tabActive, tabPast;

const cancelBooking = async (bookingId) => {
  if (!confirm('Are you sure you want to cancel this booking?')) return;
  try {
    const updated = await updateBookingStatus(bookingId, 'cancelled');
    toastSuccess('Your booking has been cancelled successfully.', 'Booking Cancelled');
...
const openRatingModal = (bookingId, providerName) => {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('rating-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    if (selectedRating === 0) {
      toastWarning('Please tap a star to rate the service before submitting.', 'Rating Required');
      return;
    }
    ...
    if (!comment.trim()) {
      setFieldError(commentEl, 'Please add a short comment about your experience.');
      return;
    }

    try {
      const updated = await rateBooking(bookingId, selectedRating, comment);
